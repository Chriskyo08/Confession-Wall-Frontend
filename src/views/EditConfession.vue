<template>
  <div class="main-box">
    <h1 class="page-title">编辑表白 #{{ confessionId }}</h1>
    
    <form @submit.prevent="handleUpdateConfession" class="confession-form">
      <!-- 表白内容 -->
      <div class="form-row">
      ...
      <div class="form-row">
        ...
        >
          <img src="@/assets/icons/paper-airplane-24.svg" alt="更新表白" class="publish-icon">
...
// 从路由获取表白ID
const confessionId = computed(() => parseInt(route.params.confessionId));

// 页面标题设置
document.title = `编辑表白 #${confessionId.value} - 表白墙`;
...
// 解析时间从 ISO 8601 格式到本地时间
const parsePublishTime = (isoString) => {
  if (!isoString) return '';
...
// 加载表白详情
const loadConfessionDetail = async () => {
  ...
  try {
    ...
    if (result.success && result.data) {
      const confession = result.data.data;
      
      // 填充表单数据
      formData.value.content = confession.content || '';
      formData.value.anonymous = confession.Anonymous || false;
      formData.value.private = confession.Private || false;
      
      // 处理图片
      if (confession.images && confession.images.length > 0) {
        previewImages.value = confession.images.map((imageUrl, index) => ({
          url: imageUrl,
      ...
      if (confession.publishedAt) {
        publishDateTime.value = parsePublishTime(confession.publishedAt);
      }
      
    } else {
      alert(result.message || '获取表白详情失败');
      router.push('/');
    }
  } catch (error) {
...
const handleUpdateConfession = async () => {
  ...
  try {
    ...
    const updateData = {
      ...
      images: formData.value.images.length > 0 ? formData.value.images[0] : null, // 根据API定义，只支持单个图片
      confessionId: confessionId.value,
      publishTime: publishDateTime.value ? formatPublishTime(publishDateTime.value) : ''
    };
    
    const result = await confessionStore.updateConfession(
      updateData.content,
      updateData.anonymous,
      updateData.private,
      updateData.images,
      updateData.confessionId,
    ...
    if (result.success) {
      alert('表白更新成功！');
      // 跳转到表白详情页
      router.push(`/confession/${confessionId.value}`);
    } else {
      alert(result.message || '更新失败，请重试');
    }
  } catch (error) {
    console.error('更新表白失败:', error);
    alert('更新失败，请重试');
  } finally {
